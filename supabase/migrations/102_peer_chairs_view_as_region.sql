-- ============================================================
-- 102 get_peer_chairs also honors viewAs region
--
-- Migration 101 let super_admin / president pass p_view_as_role to
-- preview the page as a chair. But the region filter still read
-- v_caller_region from the actual caller's profile. A super_admin's
-- profile.region is typically empty, so the region match collapsed
-- to "= '' " and matched no other chapters — zero peers despite
-- chairs being present in the same region.
--
-- This migration adds p_view_as_region as a sibling to p_view_as_role.
-- Same security gate: only super_admin / president can pass it; for
-- everyone else it's ignored.
--
-- Frontend passes the active chapter's region so a super_admin
-- viewing as Learning Chair of EO Arizona automatically gets the
-- U.S. West region context.
-- ============================================================

drop function if exists public.get_peer_chairs(text, text, text, text);
...
begin
  ...
  -- viewAs role swap (super_admin / president only)
  ...
  -- viewAs region swap — same security gate. Without this, a super_admin
  -- previewing as a chair would still filter by their own (typically
  -- empty) region and see zero peers.
  if v_can_impersonate
