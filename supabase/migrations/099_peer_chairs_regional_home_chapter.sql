-- ============================================================
-- 099 get_peer_chairs — include regional experts' home chapter
--
-- Regional experts (regional_learning_chair_expert, regional_manager)
-- do belong to a home chapter — their additional duty is overseeing
-- operations across multiple chapters in their region for their
-- specific role. The previous version of get_peer_chairs treated them
-- as chapter-less, which read as wrong. Pull profiles.chapter_id and
-- join to chapters so the frontend can show their home chapter
-- alongside the regional designation.
--
-- Also opportunistically pull phone / company from chapter_members
-- when the regional expert has a row there (most do — they're EO
-- members in their home chapter, the regional title is on top).
-- ============================================================
...
begin
  ...
  -- Chair counterparts: unchanged from migration 098
  return query
    select
      ('ra:' || ra.id::text) as id,
  ...
  -- Regional experts: now joined to their home chapter via
  -- profiles.chapter_id. Phone/company come from chapter_members
  -- when the expert is also a member of their home chapter
  -- (matched by email — same identity heuristic the rest of the
  -- app uses).
