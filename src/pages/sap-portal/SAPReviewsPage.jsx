  ...
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map(i => (
        <Star
          key={i}
          className={`h-4 w-4 ${i <= Math.round(rating) ? 'text-yellow-400 fill-yellow-400' : 'text-white/20'}`}
        />
      ))}
    </div>
  )
...
export default function SAPReviewsPage() {
    ...
    return (
      <div className="space-y-8">
        <div>
          ...
          <p className="text-sm text-white/50 mt-1">Member reviews from the Vendor Exchange</p>
        </div>
        <div className="rounded-2xl border border-white/10 bg-white/5 p-8 text-center">
          <Building2 className="h-8 w-8 text-white/20 mx-auto mb-2" />
          <p className="text-sm text-white/40">Your company hasn't been linked to the Vendor Exchange yet.</p>
          <p className="text-xs text-white/20 mt-1">Ask your chapter contact to set this up.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold">Reviews</h1>
        <p className="text-sm text-white/50 mt-1">What EO members are saying about {partner?.name}</p>
      </div>
      ...
      <div className="rounded-2xl border border-white/10 bg-white/5 p-6 flex items-center gap-4">
        <div className="text-center">
          ...
          <p className="text-xs text-white/30 mt-1">{count} review{count !== 1 ? 's' : ''}</p>
        </div>
      </div>
      ...
      {reviews.length === 0 ? (
        <div className="rounded-xl border border-white/10 bg-white/5 p-6 text-center">
          <p className="text-sm text-white/40">No reviews yet. Members can review you in the Vendor Exchange.</p>
        </div>
      ) : (
            ...
            .map(review => (
              <div key={review.id} className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-xs text-white/40">EO Member</span>
                </div>
                {review.review_text && (
                  <p className="text-sm text-white/60 mt-1">{review.review_text}</p>
                )}
